...
#if HAVE_USE_GLIB
#	include "./sqlparse/sql-tokenizer.h"
#endif
...
int
test_detect(int argc, char *argv[])
{
	char data[4096],*p;
	FILE *fp;
	int ret;
...
int test_sql(void)
{
  ...
  char * sql = " INSERT INTO mindsdb.`predictors`    (`name`, `predict`, `select_data_query`)VALUES ('bikes_model', 'count', 'SELECT * FROM test.bike_data');";
  ...
  for (k0 = 0; k0 < tokens->len; k0++) 
  {
	...
	{
	  offset += sprintf(strBuffer + offset , "where|");
	}
    else if (id == TK_SQL_INSERT)
	{
	  offset += sprintf(strBuffer + offset , "insert|");
	}
  }
...
#else
	openlog("hisql", LOG_PID, LOG_DAEMON);
#endif

   // (void)test_detect(argc, argv);     return 0;
    (void)test_sql();

    options.conf_file   =  "./hisql.conf";
...
static void handleRead(ConnectionInfo *cnx, Socket *socket, Socket *other_socket)
{
    ...
    if (cnx->remote.fd == socket->fd) {
       if (DROP == process_mysql(socket->buffer + socket->recvPos,got)) {
          handleClose(cnx, socket, other_socket);
          return;
       }
    }
...
static void handleWrite(ConnectionInfo *cnx, Socket *socket, Socket *other_socket)
{
	if (cnx->coClosing && (socket->sentPos == other_socket->recvPos)) {
		PERROR("hisql: local closed and no more output");
...
static void logEvent(ConnectionInfo const *cnx, ServerInfo const *srv, int result)
{
	...
	if (logFile) {
		if (logFormatCommon) {
			...
			fprintf(logFile, "%s - - "
				"[%s %c%.2d%.2d] "
				"\"GET /hisql-services/%s/%d/%s/%d/%s HTTP/1.0\" "
				"200 %llu - - - %llu\n",
				addressText,
				tstr,
				sign,
			...
			/* Write an hisql-specific log entry with a
				less goofy format. */
		...
		switch (c) {
			...
			case 'h':
				...
				printf("Most options are controlled through the\n"
					"configuration file. See the hisql(8)\n"
